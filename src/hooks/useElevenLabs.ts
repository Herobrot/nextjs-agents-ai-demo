...
export const useElevenLabsAgent = (
    ...
    onModeChange: ({ mode }: { mode: string }) => {
      if (mode === "speaking") {
        ...
        if (
          isEntrevistador &&
          isConnected &&
          !hasInterruptedCurrentQuestionRef.current
        ) {
    ...
    if (isConnected && isEntrevistador && conversationFlowConfig?.enabled) {
      interval = setInterval(() => {
        setConversationTime((prev) => {
          ...
          const remaining =
            (conversationFlowConfig.maxConversationDuration || 90) - newTime;
  ...
  const connect = useCallback(async () => {
    if (!elevenLabsConfig.agentId) {
      setError(
        "Agent ID no configurado. Por favor, configura el agentId en el archivo de configuración.",
      );
      return;
    }
